...
lasso.df$gender <- as.factor(lasso.df$gender)
lasso.df$duration_months <- as.numeric(lasso.df$duration_months)

xfactors <- model.matrix(codedprog ~ prpcode + dxcode + pt + cortisone_yn + gender, data=lasso.df)#[, -1]
x <- as.matrix(data.frame(lasso.df$age, lasso.df$duration_months, xfactors))

y <- as.factor(lasso.df$codedprog)  # create outcome vector
