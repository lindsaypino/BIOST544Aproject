...
lassocols <- c('codedprog', 'prpcode', 'dxcode', 'pt', 'cortisone_yn', 'age', 'gender', 'duration_months')
lasso.df <- all.df[,lassocols]
lasso.df <- lasso.df[complete.cases(lasso.df),]
lasso.matrix <- as.matrix(lasso.df)
lasso.matrix <- lasso.matrix[,-1]  # remove outcome column
outcome <- lasso.df$codedprog  # create outcome vector

# Split into train and test
train=sample(1:nrow(lasso.matrix), nrow(lasso.matrix)/2)
test=(-train)
...
lasso.mod <- glmnet(lasso.matrix[train,], outcome[train], alpha=1, family="binomial", lambda=grid)

# visualize results of the lasso, note shrinkage
plot(lasso.mod,xvar="lambda",col=c(1:ncol(x)),lwd=c(1,2,3))
legend('topright',colnames(lasso.matrix),col=c(1:ncol(lasso.matrix)),bty='n',lwd=c(1,2,3))

par(mfrow=c(1,2))
plot(ridge.mod,xvar="lambda",col=c(1:ncol(lasso.matrix)))
legend('topright',colnames(lasso.matrix),lty=1,col=c(1:ncol(lasso.matrix)),bty='n')
plot(lasso.mod,xvar="lambda",col=c(1:ncol(lasso.matrix)))

# now pick the best lambda value for the train data
set.seed(1)
cv.out=cv.glmnet(lasso.matrix[train,],outcome[train],alpha=1)
plot(cv.out)
bestlam <- cv.out$lambda.min

# fit the model to the test data and check the regression performance
lasso.pred <- predict(lasso.mod,s=bestlam,newx=lasso.matrix[test,])
mean((lasso.pred-outcome.test)^2)

out <- glmnet(lasso.matrix,outcome,alpha=1,lambda=grid)
